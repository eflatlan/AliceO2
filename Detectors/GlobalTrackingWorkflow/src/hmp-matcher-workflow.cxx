...
void customize(std::vector<o2::framework::ConfigParamSpec>& workflowOptions)
{
  ...
  std::vector<o2::framework::ConfigParamSpec> options{
    ...
    {"track-sources", VariantType::String, std::string{GID::ALL}, {"comma-separated list of sources to use: allowed TPC,ITS-TPC,TPC-TRD,ITS-TPC-TRD (all)"}}}; // }}, ef
...
*/
  ...
  LOG(debug) << "HMP track inputs = " << configcontext.options().get<std::string>("track-sources");
  LOG(debug) << "HMP output = " << outputType;
 ...
 // LOG(debug) << "TOF matching in strict mode = " << strict;
  ...
  auto strict = true; // ef: just set strict true to compile
  ...
  if (!disableRootOut) {
    ...
    if (writematching) { 
      if (GID::includesSource(GID::TPC, src)) { // matching to TPC was requested
       /*
        // ef: no method getHMPMatcherWriterSpec in o2::hmpid namespace
        writers.emplace_back(o2::hmpid::getHMPMatcherWriterSpec(useMC, "o2match_hmp_tpc.root", true, (int)o2::dataformats::MatchInfoHMP::TrackType::TPC, strict)); */
      }
      if (GID::includesSource(GID::ITSTPC, src)) { // matching to ITS-TPC was requested, there is not strict mode in this case
       /* writers.emplace_back(o2::hmpid::getHMPMatcherWriterSpec(useMC, "o2match_hmp_itstpc.root", false, (int)o2::dataformats::MatchInfoHMP::TrackType::ITSTPC, false)); */
      }
      if (GID::includesSource(GID::TPCTRD, src)) { // matching to TPC-TRD was requested, there is not strict mode in this case
       /* writers.emplace_back(o2::hmpid::getHMPMatcherWriterSpec(useMC, "o2match_hmp_tpctrd.root", false, (int)o2::dataformats::MatchInfoHMP::TrackType::TPCTRD, strict));
      */}
      if (GID::includesSource(GID::ITSTPCTRD, src)) { // matching to ITS-TPC-TRD was requested, there is not strict mode in this case
      /*  writers.emplace_back(o2::hmpid::getHMPMatcherWriterSpec(useMC, "o2match_hmp_itstpctrd.root", false, (int)o2::dataformats::MatchInfoHMP::TrackType::ITSTPCTRD, false));
      */}
    }
  /*  if (writecalib) {
